...
open class CacheableProguardTask @Inject constructor(
    ...
    @Optional
    @OutputFile
    fun getPrintConfiguration(): File? = configuration.printConfiguration?.takeIf { it.name.isNotEmpty() }

    @Input
    internal fun getPrintConfigurationToStdOut(): Boolean = configuration.printConfiguration?.name?.isEmpty() ?: false
