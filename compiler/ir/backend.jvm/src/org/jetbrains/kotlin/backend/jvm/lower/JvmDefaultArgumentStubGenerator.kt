...
class JvmDefaultArgumentStubGenerator(override val context: JvmBackendContext) : DefaultArgumentStubGenerator(context, false, false) {
    ...
    override fun needsSuperCallHandlerCheck(irFunction: IrSimpleFunction): Boolean =
        !irFunction.isTopLevelDeclaration && (irFunction.parent as? IrClass)?.isFinalClass != true

    override fun IrBlockBodyBuilder.generateSuperCallHandlerCheck(
        ...
        +irIfThen(
            ...
            irThrow(irCall(this@JvmDefaultArgumentStubGenerator.context.ir.symbols.ThrowUnsupportOperationExceptionClass).apply {
                putValueArgument(
                    0,
                    irString("Super calls with default arguments not supported in this target, function: ${irFunction.name.identifier}")
                )
            })
        )
    }
